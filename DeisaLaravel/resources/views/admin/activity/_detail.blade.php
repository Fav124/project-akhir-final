...
<div class="p-6 space-y-4">
    <div class="flex items-center gap-4">
        <div class="w-14 h-14 rounded-2xl bg-blue-50 text-deisa-blue flex items-center justify-center text-2xl shadow-sm border border-blue-100">
            @php
        ...
        <div>
            <div class="font-black text-slate-900 text-lg tracking-tight">{{ ucfirst($log->action) }} in {{ ucfirst($log->module) }}</div>
    ...
    <div class="grid grid-cols-2 gap-4 text-sm mt-4">
        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">User Pelaksana</label>
    ...
    <div class="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-inner">
        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-3">Payload / Detail Data</label>
        <pre class="text-xs text-emerald-400 overflow-x-auto whitespace-pre-wrap leading-relaxed font-mono">{{ $log->detail }}</pre>
    </div>
</div>
