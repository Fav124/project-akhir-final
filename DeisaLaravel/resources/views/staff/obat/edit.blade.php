...
@section('content')
    <div class="min-h-screen bg-slate-50  py-8 ">
        <div class="max-w-3xl mx-auto px-6">
            <div class="mb-8">
                <h1 class="text-2xl font-black text-slate-800 ">Edit Obat</h1>
                <p class="text-sm text-slate-500 ">Perbarui data obat</p>
            </div>

            <div
                class="bg-white  rounded-2xl border border-slate-100  p-8 ">
                <form action="{{ route('staff.obat.update', $obat->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="keterangan" rows="3"
                                class="w-full px-4 py-3 rounded-xl border border-slate-200  bg-white  text-slate-800  focus:ring-2 focus:ring-deisa-blue focus:border-transparent transition-colors">{{ old('keterangan', $obat->keterangan) }}</textarea>
                        </div>
                    </div>
