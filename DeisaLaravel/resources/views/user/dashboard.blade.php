...
@section('content')
    <div class="min-h-screen pb-24 bg-slate-50">
        <!-- Header -->
        <header class="bg-gradient-to-br from-deisa-blue to-indigo-700 relative overflow-hidden shadow-2xl">
                ...
                <div class="relative z-10 flex justify-between items-start mb-6">
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <span class="px-2 py-0.5 bg-white/20 rounded text-[10px] font-bold text-white uppercase tracking-wider">Petugas Mode</span>
                        </div>
                    ...
                    <div class="relative flex items-center gap-3" x-data="{ 
                    ...
                    }" x-init="fetchReminders(); setInterval(() => fetchReminders(), 30000)" @refresh-reminders.window="fetchReminders()">
                        <!-- Daily Summary Button -->
                        <button @click="$dispatch('open-focus-mode')" class="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded-xl text-sm font-bold transition-all backdrop-blur-md">
                        ...
                        <!-- Notification Bell -->
                        <div class="relative">
                            <button @click="notifications.open = !notifications.open" class="w-10 h-10 rounded-xl bg-white/10 border border-white/20 flex items-center justify-center backdrop-blur-md hover:bg-white/20 transition-all relative">
                                <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
                                </svg>
                                <span x-show="notifications.count > 0" class="absolute top-2 right-2 w-2 h-2 bg-rose-500 rounded-full"></span>
                            </button>
                            ...
                            <div x-show="notifications.open" @click.away="notifications.open = false" x-cloak
                                ...
                                <div class="p-4 bg-slate-50 border-b border-slate-100">
                                    <span class="text-xs font-black text-slate-400 uppercase tracking-widest">Pengingat</span>
                                </div>
                                <div class="max-h-80 overflow-y-auto">
                                    <template x-if="notifications.items.length === 0">
                                        <div class="p-6 text-center">
                                            <p class="text-[10px] text-slate-400 font-bold uppercase">Tidak ada pengingat</p>
                                        </div>
                                    </template>
                                    <template x-for="item in notifications.items" :key="item.id">
                                        <div class="p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors flex gap-3">
                                            <div class="flex-1">
                                                <div class="text-xs font-bold text-slate-800" x-text="item.title"></div>
                                                <div class="text-[10px] text-slate-400" x-text="item.message"></div>
                                            </div>
                        ...
                        <button onclick="window.location.reload()" class="w-11 h-11 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center backdrop-blur-md hover:bg-white/20 transition-all">
                            <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="-mt-14 mb-10 relative z-10">
                <div class="bg-white/70 backdrop-blur-xl rounded-[2rem] shadow-2xl shadow-blue-900/10 p-1 border border-white/50">
                    <div class="flex flex-wrap justify-between items-center p-8 md:p-12 bg-white rounded-[1.8rem]">
                        <div class="flex-1 min-w-[150px] text-center mb-6 md:mb-0">
                            <div class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2">Pasien Hari Ini</div>
                            <div class="text-4xl md:text-6xl font-black text-slate-800">{{ $pasienHariIni }}</div>
                        </div>
                        <div class="hidden md:block h-20 w-px bg-slate-100 mx-8"></div>
                        <div class="flex-1 min-w-[150px] text-center">
                            <div class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2">Menunggu Sembuh</div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="space-y-4">
                        @forelse($activePatients as $sakit)
                            <div class="bg-white rounded-[1.8rem] p-5 border border-slate-100 shadow-sm hover:shadow-md transition-all">
                                <div class="flex items-center gap-4 mb-4">
                                    ...
                                    <div class="flex-1">
                                        <h4 class="font-bold text-slate-800 text-lg leading-tight">{{ $sakit->santri->nama_lengkap }}</h4>
                                        <div class="flex items-center gap-2 mt-1">
                                            <span class="text-xs font-medium text-slate-400">Kelas {{ $sakit->santri->kelas->nama_kelas }}</span>
                                            <span class="w-1 h-1 bg-slate-200 rounded-full"></span>
                                            <span class="text-[10px] font-bold text-deisa-blue uppercase tracking-wider">{{ $sakit->created_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex gap-3">
                                    <button 
                                        ...
                                        data-confirm="Konfirmasi pasien telah sembuh?"
                                        data-reload="true"
                                    ...
                                    <button class="w-12 h-12 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-slate-400 hover:text-deisa-blue hover:border-deisa-blue transition-all"
                        ...
                        @empty
                            <div class="bg-slate-100/50 rounded-[2.5rem] p-12 text-center border-2 border-dashed border-slate-200">
                                ...
                                <p class="text-xs text-slate-400 max-w-[200px] mx-auto leading-relaxed">Saat ini tidak ada santri yang sedang dalam perawatan UKS.</p>
                            </div>
                        @endforelse
                    </div>
                        ...
                        @foreach($recentPatients as $recent)
                            <div class="p-4 border-b border-slate-50 last:border-0 flex items-center gap-4 hover:bg-slate-50/50 transition-colors cursor-pointer"
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="text-[10px] text-slate-400">{{ $recent->created_at->format('d M H:i') }} • {{ $recent->status }}</div>
                                </div>
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <a href="{{ route('user.sakit.create') }}"
                                ...
                                <div class="flex items-center gap-5">
                                    <div class="bg-rose-50 text-rose-500 p-4 rounded-2xl group-hover:bg-rose-500 group-hover:text-white transition-colors duration-300">
                            ...
                            <a href="{{ route('user.obat.list') }}"
                                class="group relative overflow-hidden bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                                <div class="absolute -right-4 -bottom-4 w-24 h-24 bg-deisa-blue/5 rounded-full group-hover:scale-150 transition-transform duration-500"></div>
                                <div class="flex items-center gap-5">
                                    <div class="bg-blue-50 text-deisa-blue p-4 rounded-2xl group-hover:bg-deisa-blue group-hover:text-white transition-colors duration-300">
                                        <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div class="p-6 bg-gradient-to-br from-slate-800 to-slate-900 rounded-[2rem] text-white overflow-hidden relative">
                                ...
                                <p class="text-xs text-slate-400 mb-4 leading-relaxed">Gunakan fitur ini untuk melaporkan kerusakan stok atau kendala teknis.</p>
                                <button class="w-full py-2 bg-white/10 hover:bg-white/20 rounded-xl text-xs font-bold transition-all border border-white/10">Buka Support</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <nav class="glass-effect rounded-[2rem] border border-white/40 shadow-2xl p-2 flex justify-around items-center">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" @click.away="open = false" 
                        ...
                        <a href="{{ route('profile.index') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 font-medium">
                            Profil Saya
                        </a>
                        <hr class="border-slate-50 my-1">
                        <form action="{{ route('user.logout') }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 text-sm text-rose-600 hover:bg-rose-50 font-bold">
                                Keluar Aplikasi
                            </button>
                        </form>
                    </div>
