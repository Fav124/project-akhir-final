<div x-data="{ 
    ...
    <div class="flex gap-1 p-1 bg-slate-100 rounded-2xl w-fit">
        <button @click="tab = 'info'"
            :class="tab === 'info' ? 'bg-white text-deisa-blue shadow-sm' : 'text-slate-500 hover:text-slate-700'"
            class="px-6 py-2 rounded-xl text-sm font-bold transition-all">Pribadi</button>
        <button @click="tab = 'appearance'"
            :class="tab === 'appearance' ? 'bg-white text-deisa-blue shadow-sm' : 'text-slate-500 hover:text-slate-700'"
            class="px-6 py-2 rounded-xl text-sm font-bold transition-all">Tampilan</button>
        <button @click="tab = 'security'"
            :class="tab === 'security' ? 'bg-white text-deisa-blue shadow-sm' : 'text-slate-500 hover:text-slate-700'"
            class="px-6 py-2 rounded-xl text-sm font-bold transition-all">Keamanan</button>
    </div>

    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" data-ajax="true"
        data-reload="true" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <x-card class="border-0 shadow-sm">
                <div class="flex flex-col md:flex-row items-center gap-8">
                    ...
                    <div x-data="{ photoName: null, photoPreview: null }">
                        <input type="file" class="hidden" x-ref="photo" name="avatar" @change="
                        ...
                        <div class="relative group cursor-pointer" @click="$refs.photo.click()">
                            <div
                                class="w-32 h-32 rounded-3xl bg-blue-50 border-4 border-white shadow-lg overflow-hidden flex items-center justify-center">
                                <template x-if="!photoPreview">
                                    @if($user->profile?->avatar)
                                        <img src="{{ Storage::url($user->profile->avatar) }}"
                                            class="w-full h-full object-cover">
                                    @else
                                        <div class="text-4xl font-black text-deisa-blue">{{ substr($user->name, 0, 1) }}
                                        </div>
                                    @endif
                                </template>
                                <template x-if="photoPreview">
                            ...
                            <div
                                class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 rounded-3xl flex items-center justify-center transition-all">
                    ...
                    <div class="flex-1 space-y-4 w-full">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label
                                    class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Nomor
                                    Telepon</label>
                        ...
                        <div>
                            <label
                                class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Bio
                                Singkat</label>
        ...
        <div x-show="tab === 'appearance'" x-cloak x-transition:enter="transition ease-out duration-200"
            x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
            <x-card class="border-0 shadow-sm">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    @foreach($colors as $val => $info)
                        <label class="relative cursor-pointer group">
                            <input type="radio" name="theme_color" value="{{ $val }}" class="sr-only"
                            ...
                            <div class="p-4 rounded-3xl border-2 transition-all duration-300"
                                ...
                                <div
                                    class="w-full h-16 rounded-2xl {{ $info['bg'] }} mb-3 shadow-inner relative overflow-hidden">
                                    <div
                                    ...
                                    <div x-show="selectedTheme === '{{ $val }}'"
                                        ...
                                        <div
                                            class="w-8 h-8 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center shadow-lg border border-white/30">
        ...
        <!-- TAB: KEAMANAN -->
        <div x-show="tab === 'security'" x-cloak x-transition:enter="transition ease-out duration-200"
            x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
            <x-card class="border-0 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4">
                    <div x-data="{ show: false }">
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Kata
                            Sandi Saat Ini</label>
                        <div class="relative">
                            <input :type="show ? 'text' : 'password'" name="current_password"
                                class="w-full pl-4 pr-11 py-2 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500/20 focus:border-deisa-blue outline-none transition-all">
                    ...
                    <div x-data="{ show: false }">
                        <label
                            class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Konfirmasi
        ...
        <div class="pt-4">
            <x-button type="submit" class="w-full py-4 rounded-2xl shadow-lg shadow-blue-500/20">Simpan Seluruh
                Perubahan</x-button>
        </div>
    </form>
</div>